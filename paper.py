...
class ArxivPaper:
    ...
    def tldr(self) -> str:
        ...
        llm = get_llm()
        prompt = """请根据提供的论文信息（标题、摘要、引言和结论），**严格按照以下结构、标签和换行要求**，用中文生成总结：

0.  **短语分类：** 直接输出一个精炼的中文短语分类标签（例如：LLM 量化；VLM token 剪枝；投机解码），并在该短语前后加上方括号，作为总结的第一行。
1.  **[问题]**：总结本篇工作是解决大型模型（LLM/CV/NLP/AI等细分领域）的什么具体问题。
2.  **[缺陷]**（可选）：总结现有方法的主要缺陷或不足。如果论文信息中未明确提及缺陷或无法合理推断出，**请省略此句话，不要生成，并将后续句子的标签和序号顺延。**
3.  **[启发]**：阐述作者获得了什么核心启发，促成了这项工作的诞生。
4.  **[方法]**：用**一到两句话**详细描述本文提出的核心方法和关键技术细节。**如果包含两句话，请确保这两句话是连续的，中间不要换行，也不要重复 [方法] 标签。**
5.  **[结论]**：总结实验效果，并得出最终结论。

请确保：
- 总结的第一行是第 0 句的短语分类。
- 除方法描述外，其他每一句都以指定的标签开头（例如：[问题]），并独立换行。方法描述（第 4 句）作为一个整体，只在开头有一次 [方法] 标签，并且在方法描述结束时才换行。
- 如果省略了 [缺陷] 句，则总共输出五句话加一个分类标签（总共六行）；如果包含 [缺陷] 句，则总共输出六句话加一个分类标签（总共七行）。
...
__CONCLUSION__
        ...
        tldr = llm.generate(
            messages=[
                {
                    "role": "system",
                    "content": "您是专业的科研论文总结助手，您的任务是根据提供的论文信息，严格按照用户指定的结构、标签和条件，用中文生成一篇精确、详尽、且格式规范的总结。请确保每句话都单独换行，并以指定的标签开头。",
                },
